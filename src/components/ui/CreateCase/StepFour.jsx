...
import { createNewPlatform, createNewPosting, createNewProfileHandle } from "@/lib/actions";
import { type } from "os";
...
const StepFour = ({ platforms,profileHandles, prevStep, nextStep }) => {
  ...
  const handleNext = async() => {
    setloading(true)
    const perpetratorID=localStorage.getItem('NewPerpetratorID')
    if(newPlatform && newProfileHandle && postText && postDesc){
      const resp=await createNewPosting(postText,postDesc,newPlatform,perpetratorID,newProfileHandle,caseId)
      if(resp){
        nextStep()
      }
      else{
  ...
  return (
    <div className="p-6 bg-white rounded-xl border">
      <h2 className="text-lg font-medium text-gray-700 mb-4">Step 4: Create Posting</h2>
